...
//=================
//Menu
let iconMenu = document.querySelector(".navbar-toggler");
if (iconMenu != null) {
    let delay = 500;
...
function popup_open(item, video = '') {
    ...
    document.querySelector('#popup__bg').classList.add('bg_active');
    let activePopup = document.querySelectorAll('.popup._active');
    if (activePopup.length > 0) {
        popup_close('', false);
    }
    let curent_popup = document.querySelector('.' + item);
...
function popup_close(item, bodyUnlock = true) {
    if (unlock) {
        if (!item) {
            document.querySelector('#popup__bg').classList.remove('bg_active');
            for (let index = 0; index < popups.length; index++) {
                const popup = popups[index];
